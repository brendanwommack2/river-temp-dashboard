...
// plain YAML/routing, not JS, so they can't read this variable — see
// the note at the end of this file for how to handle those.)
...
const trendSummary = yoyDelta !== null
  ? `Through today, ${currentYear} is tracking ${Math.abs(yoyDelta).toFixed(1)}°C ${yoyDelta > 0 ? "warmer" : "cooler"} than ${priorYear} at this same point in the season.`
